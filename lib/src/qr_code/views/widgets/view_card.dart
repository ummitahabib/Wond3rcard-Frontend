...
//   final isLoading = card == null || profileController.profileData == null;
...
//       child: _buildCardUI(context, cardController), // Move UI to separate widget
//     ),
//   );
// }


//   Center _buildCardUI(BuildContext context, CardNotifier cardController) {
//     return Center(
//         child: Container(
...
class ViewCard extends HookConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final cardList = cardController.getCardsResponse?.payload?.cards ?? [];

    // ⚠️ Check loading and index validity
    if (isDataLoading.value || index >= cardList.length) {
      return Scaffold(
        appBar: AppBar(
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        backgroundColor: AppColors.primaryShade,
        leading: IconButton(
          icon: const Icon(Icons.arrow_back_ios, color: AppColors.defaultWhite),
          onPressed: () => context.go(RouteString.shareCardLink),
        ),
      ),
      floatingActionButton: CircleAvatar(
        backgroundColor: AppColors.primaryShade,
        child: IconButton(
          icon: const Icon(Icons.download, color: AppColors.defaultWhite),
          onPressed: () {},
      ...
      body: ExportFrame(
        frameId: 'card_frame',
        exportDelegate: ExportDelegate(),
        child:  Container(child: Text('Hello Ummeeta'),)
        //_buildCardUI(context, card),
      ),
    );
  }
